# 🐾 C4TB0T
C4TB0T Trader Calculator for DayZ Xbox One 1.28
Created and maintained by **BL4CKC4T87**

Trackz ur buys and sells, calculatez ur balances, generatez ur trade invoices.
Bring C4TB0T catfood plz.

This tool is fully free to use. Prices and items are customizable.
Please read the accompanying LICENSE — this tool may not be bought, sold, or redistributed in any way other than as defined therein.

---

## 📥 Download

Get the latest official build from the Discord community.
GitHub address is available on the Discord.

- Discord: [BL4CKC4T87](https://discord.gg/x4jGSbg2NA)

> ⚠️ Only download from official links to ensure safety and authenticity.

**File naming convention:**
`C4TB0T(<user-variable>).html`
(where <user-variable> is optional for personal labeling)

---

## 💡 Features

- Fully free — same features for all users
- Transparent, safe, and easy to use
- Prices and items are customizable
- Clear branding in filename and UI

---

## 🐱 Donations

Completely optional. Donations do not unlock features, early access, or any in-game advantages — they're simply a way to support the project.

---

## 📜 License & Attribution

Provided under the **C4TB0T Tool License v1.7**.

- Personal and public non-commercial use permitted with attribution
- Redistribution allowed — must be free, credited, and unmodified (or clearly noted as modified)
- The name C4TB0T and author credit must remain attached
- See [LICENSE.txt](LICENSE.txt) for full terms

---

## 💬 Support & Community

- Discord: [BL4CKC4T87](https://discord.gg/x4jGSbg2NA)
- GitHub address available on the Discord

---

## 🤖 Acknowledgments

Some HTML and formatting contributions were assisted by **Claude AI**.

---

## ⚠️ Disclaimer

C4TB0T is provided "as is." The author is not responsible for any damages, data loss, or issues resulting from its use.

---

C4TB0T — Created by BL4CKC4T87
Discord: discord.gg/x4jGSbg2NA
